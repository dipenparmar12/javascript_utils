 ...
 */
function find(arrOfObj, key, values, ...args) {
  Array.isArray(values) || (values = [values])
  if (args) values.splice(values.length, 0, ...args) // merge arrays
  return arrOfObj && arrOfObj.find((el) => values.includes(el[key]))
}
...
find(testInventory, 'name', 'apple') // [{name: 'apple', qty: 3, dry: true}]
find(testInventory, 'qty', [0, 2]) // [{name: 'apples', qty: 2, dry: true}, {qry:0,...}]
find(testInventory, 'qty', [10]) // [{name: 'kivi', qty: 10, dry: true}]
find(testInventory, 'dry', true) // all obj those have dry=true
